...
echo "---------------------------------------------------------------"
pacman -Syu --noconfirm patchelf libnss_nis nss-mdns nss bubblewrap
...
link=$(curl -sL --compressed "$BASE_URL/download" | grep -oP "https://storage\.googleapis\.com[^\"]+linux-$farch/Antigravity\.tar\.gz")
